...
if (fs.existsSync("./Engine/Plugins/Settings/startTime.json")) {
    var startTime = DB.load("./Engine/Plugins/Settings/startTime.json").startTime;
} else {
    var startTime = 0;
    ...
    commands.startstream = function(channel, user, message, command, isPrivate) {
        ...
        if (options.admins.includes(user.username)) {
            ...
            if (time > 0) {
				...
				//save the start time.
                DB.save("./Engine/Plugins/Settings/startTime.json", {
                    startTime: startTime
                })
				//clear any old countdowns
                clearInterval(countDownInterval)
