...
var savetimeout
if (fs.existsSync("./Engine/Plugins/Settings/Playlist.json")) {
    var playlist = DB.load("./Engine/Plugins/Settings/Playlist.json")
} else {
    var playlist = {}
    DB.save("./Engine/Plugins/Settings/Playlist.json", playlist)
}

if (fs.existsSync("./Engine/Plugins/Settings/SongRequests.json")) {
    var songsToPlay = DB.load("./Engine/Plugins/Settings/SongRequests.json")
} else {
    var songsToPlay = []
    DB.save("./Engine/Plugins/Settings/SongRequests.json", songsToPlay)
}
if (fs.existsSync("./Engine/Plugins/Settings/CurrentSong.json")) {
    var currentSong = DB.load("./Engine/Plugins/Settings/CurrentSong.json")
} else {
    var currentSong = {
    ...
    DB.save("./Engine/Plugins/Settings/CurrentSong.json", songsToPlay)
}
if (!fs.existsSync("./Engine/Plugins/Settings/CurrentSong.json")) {
    DB.save("./Engine/Plugins/Settings/CurrentSong.json", "None")
}
if (fs.existsSync("./Engine/Plugins/Settings/YoutubeMusic.json")) {
    var settings = DB.load("./Engine/Plugins/Settings/YoutubeMusic.json")
    ...
    DB.save("./Engine/Plugins/Settings/YoutubeMusic.json", settings)
}
...
function nextSong(channel, say, display) {
    if (songsToPlay.length > 0) {
        ...
        DB.save("./Engine/Plugins/Settings/CurrentSong.json", currentSong)
        audioPlaying = true
        DB.save("./Engine/Plugins/Settings/SongRequests.json", songsToPlay)
    } else {
...
function removeSong(channel, user, command, say, isPrivate, display, options) {
    ...
    if (options.admins.includes(user.username)) {
        if (songsPlayed[currentSong.src] != null) {
            ...
            DB.save("./Engine/Plugins/Settings/Playlist.json", Object.assign(playlist, songsPlayed))
        } else {
                            ...
                            if (error) {
                                ...
                                if (result.items.length > 0 && result.items[0].id != null && result.items[0].id.videoId != null) {
                                    if (songsToPlay.length == 0 && !audioPlaying) {
                                        ...
                                        DB.save("./Engine/Plugins/Settings/CurrentSong.json", {
                                            user: user.username,
                                            src: "https://www.youtube.com/watch?v=" + result.items[0].id.videoId,
                                            title: result.items[0].snippet.title
                                        })
