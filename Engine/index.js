...
var markovBrainPath = "./Engine/markov/MarkovDB.json";

Logging.setNamespace("TwitchBot");
Logging.setConsoleLogging(true);

//Load DBS
if (fs.existsSync("./Engine/Options.json")) {
    var options = DB.load("./Engine/Options.json");
} else {
    var options = {
        options: {
            debug: false
        ...
        viewerDbPath: "./Engine/DB/viewerDB.json",
        statsDbPath: "./Engine/DB/statsDB.json",
        playerDeleteTimeoutDays: 30,
        botIsPaused: false,
...
function feedMarkov(user, message) {
    if (!user.includes("bot") && user != options.identity.username.toLowerCase() && !options.ignoredUsernames.includes(user.username) && message.charAt(0) != "!" && !message.includes("*") && !message.includes("/") && !message.includes("http") && !message.includes("@") && message.length < options.markovLearningMaxLength) {
...
function log(str, isError = false, nameSpace = "TwitchBot") {
        ...
        if (message.charAt(0) === "!") {
            if (user.mod || isAdmin(user.username) || isPrivate || new Date().getTime() >= viewerDB.Viewers[user.username].nextCommandAllowed) {
                ...
                if (command == "about") {
                    ...
                    say(user, channel, "This is a bot created by dmhzmxn.", isPrivate)
                    ...
                    DB.save("./Engine/Options.json", options)
...
function init(newio) {
    ...
    io.on('connection', function (socket) {
        ...
        log("[" + socket.request.connection.remoteAddress + "] Connected to overlay controller! ", false, "Overlay");
        io.connectioncount++
            io.clientcount++
            //generate users sessionID to prevent man in middle
            socket.sessionID = io.generate_key()
        socket.on('disconnect', function (data) {
            log("[" + this.request.connection.remoteAddress + "] Disconnected from overlay controller.", false, "Overlay");
            io.clientcount--
        });
    ...
    client.on("join", function (channel, user, self) {
        if (!options.markovLearningChannels.includes(channel)) {
            ...
            if (user != options.identity.username.toLowerCase() && !options.ignoredUsernames.includes(user)) {
                ...
                viewerDB.Viewers[user].playerDeleteTimeout = new Date().getTime() + ((((options.playerDeleteTimeoutDays * 24) * 60) * 60) * 1000);
                if (options.subsGetDoublePointGain && viewerDB.Viewers[user].subscribed) {
    ...
    client.on("part", function (channel, username, self) {
        if (!options.markovLearningChannels.includes(channel)) {
            ...
            if (viewerDB.Viewers[username] != null) {
                viewerDB.Viewers[username].playerDeleteTimeout = new Date().getTime() + ((((options.playerDeleteTimeoutDays * 24) * 60) * 60) * 1000);
            }
        }
    });
            ...
            if (user.username != options.identity.username.toLowerCase() && !options.ignoredUsernames.includes(user.username)) {
                ...
                if (!options.markovLearningChannels.includes(channel)) {
                    if (user != null) {
                        ...
                        viewerDB.Viewers[user.username].playerDeleteTimeout = new Date().getTime() + ((((options.playerDeleteTimeoutDays * 24) * 60) * 60) * 1000);
                        viewerDB.Viewers[user.username].nextChatAllowed = new Date().getTime() + options.chatSpamTimeout;
                    }
